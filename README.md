...
| `37adfd719` | **Reference production node config:** `contrib/qogecoin-production.conf.example` — captures every operational lesson learned during development and testnet operation. Documented: `changetype=bech32` (forces SegWit v0 change outputs; Taproot change exposes the raw public key at rest, wrong for a PQC project); `fallbackfee`/`paytxfee`/`mintxfee` (fee estimation silently fails on low-activity chains without these); `wallet=` persistence (a wallet loaded via `loadwallet` RPC is not reloaded on restart — silent `-18` errors after every daemon restart). |
| `dbea00cc7` | **Build fixes:** (1) `configure.ac` — liboqs Option A prefix detection used `$prefix` (always empty/NONE in the depends workflow) instead of `$depends_prefix` (set by `config.site.in`); corrected to check `depends_prefix` first, falling back to `$prefix`/`$ac_default_prefix` for non-depends builds. Cosmetic fix only — `config.site.in`'s `PKG_CONFIG_PATH` redirection masked the bug in practice, but this removes reliance on that implicit behavior. (2) `depends/packages/boost.mk` — primary source `boostorg.jfrog.io` is permanently dead (JFrog discontinued free Boost hosting); synced to `archives.boost.io` (matching upstream `bitcoin/bitcoin@ffbc173ca1`'s own fix) and added a QOGE-hosted GitHub release as a Boost-specific fallback tier via `$(package)_fetch_cmds`. sha256 hash unchanged. Empirically tested (forced tier-1 failure, confirmed clean fallback). Independently reviewed by Codex and Grok Build — zero findings. |
| `13bd888d3` | **Build doc reconciliation:** `doc/build-ubuntu-2404.md` added — the only current, tested build path for this fork, validated end-to-end on fresh Ubuntu 24.04 hardware. All seven upstream platform build docs (`build-android.md`, `build-freebsd.md`, `build-netbsd.md`, `build-openbsd.md`, `build-osx.md`, `build-unix.md`, `build-windows.md`) renamed to `*-DEPRECATED.md` — kept for reference, untested and unsupported for this fork. `doc/README.md` updated to link to the new tutorial and mark the deprecated docs accordingly. |
...
- **Reference production node config: [`contrib/qogecoin-production.conf.example`](contrib/qogecoin-production.conf.example)** (`37adfd719`) — copy and edit before deploying a fresh mainnet node. Documents `changetype=bech32` (Taproot change exposes raw pubkey at rest — wrong for a PQC project), fee fallback settings (silent failure on low-activity chains), and wallet-persistence pitfall (wallets loaded via RPC are not reloaded on restart).
